...
import { GALLERY_ITEMS, PAGE_HEADERS } from '@/constant/constants';
                            ...
                            {filteredItems.map((item) => (
                                ...
                                >
                                    ...
                                    <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/80 to-transparent translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                                        <p className="text-xs text-silver font-bold uppercase mb-1">{item.category}</p>
            ...
            <AnimatePresence>
                {selectedImage && (
                    ...
                    >
                        <Button
                            variant="ghost"
                            className="absolute top-6 right-6 text-white/50 hover:text-white transition p-2 bg-white/10 rounded-full border-none h-auto w-auto z-50"
                            onClick={() => setSelectedImage(null)}
                        >
                            <X size={24} />
                        </Button>
